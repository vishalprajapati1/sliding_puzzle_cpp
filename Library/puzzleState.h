...
// Overloads for pointer

 //bool operator==(PuzzleState* state) { 
 //  // assuming same size puzzle is compared
 //  for (int i = 0; i < this->puzzle.size(); i++) {
 //    for (int j = 0; j < this->puzzle.size(); j++) {
 //      if (this->puzzle[i][j] != state->puzzle[i][j]) return false;
 //    }
 //  }
 //  return true;
 //}
 ...
 // PuzzleState* move(int direction) {
//  // generates a new puzzle
//  PuzzleState* state = new PuzzleState(this->puzzle, this->cavity);
//  int& cavity_x = state->cavity.first, cavity_y = state->cavity.second;
//  if (direction == 0) {
//    std::swap(state->puzzle[cavity_x][cavity_y],
//    state->puzzle[cavity_x][cavity_y + 1]);
//    ++cavity_y;
//  }
//  if (direction == 1) {
//    std::swap(state->puzzle[cavity_x][cavity_y],
...
//  if (direction == 2) { // up
//    std::swap(state->puzzle[cavity_x][cavity_y],
//              state->puzzle[cavity_x + 1][cavity_y]);
//    ++cavity_x;
//  }
//  if (direction == 3) { // down
//    std::swap(state->puzzle[cavity_x][cavity_y],
...
// PuzzleState(PuzzleState* obj) {
//  std::cout << "copy cc called";
//  this->cavity = obj->cavity;
//  // deep copy of vector
//  for (int i = 0; i < obj->puzzle.size(); i++) {
//    this->puzzle.push_back(obj->puzzle[i]);
//  }
//}